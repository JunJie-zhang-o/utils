    ...
    def createTrackbar(self):
        for k in self.params.keys():
            # if type(params[k]) == IntParam :
                cv2.createTrackbar(k, self.WINDOWS_NAME, self.params[k].value, self.params[k].max, self.updateParam)
    ...
    def updateParam(self, arg):
        for k in self.params.keys():
            ...
            if v != self.params[k].value:
                if k.upper() in V4l2ToCv2.__members__.keys() and V4l2ToCv2[k.upper()].value is not None :
                    self.params[k].value = v
                    print(f"设置相机参数 | {k}:{v} | default:{self.params[k].default}")
                    self.cap.set(V4l2ToCv2[k.upper()].value, v)
                else:
                    print(f"未能正确找到设置该参数的接口 | {k}:{v} | default:{self.params[k].default}")
